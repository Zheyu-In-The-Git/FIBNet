...
class LFWInterface(pl.LightningDataModule):
    ...
    def prepare_data(self):
        LFWData(dim_img=self.dim_img, data_dir=self.data_dir, identity_nums=self.identity, sensitive_attr=self.sensitive_attr, img_path_replace=self.LFWData_img_path_replace)
    ...
    def setup(self, stage=None):
        if stage == 'fit' or stage is None:
            self.trainset = LFWData(dim_img=self.dim_img, data_dir=self.data_dir, identity_nums=self.identity, sensitive_attr=self.sensitive_attr, img_path_replace=self.LFWData_img_path_replace)
            self.valset = LFWData(dim_img=self.dim_img, data_dir=self.data_dir, identity_nums=self.identity, sensitive_attr=self.sensitive_attr, img_path_replace=self.LFWData_img_path_replace)

        if stage == 'test' or stage is None:
            self.testset = LFWRecognitionTestPairs(dim_img=self.dim_img, data_dir=self.data_dir, img_path_replace=self.LFWRecognition_img_path_replace)
    ...
    def val_dataloader(self):
        return DataLoader(self.valset, batch_size=self.batch_size, num_workers=self.num_workers, shuffle=True, pin_memory=self.pin_memory)
...
if __name__ == '__main__':
    ...
    dataloader = LFWInterface(dim_img=224, dataset='LFW_data', data_dir=data_dir, sensitive_attr='Male', batch_size=2, num_workers=0, pin_memory=False, identity_nums=1680, sensitive_dim=1)
