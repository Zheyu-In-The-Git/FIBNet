...
class LFWCasiaMineData(data.Dataset):
    def __init__(self,
                 dim_img:int,
                 lfw_data_dir:str,
                 casia_data_dir:str,
...
if __name__ == '__main__':
    ...
    lfw_casia_dataset = LFWCasiaMineData(dim_img=224, lfw_data_dir=lfw_data_dir, casia_data_dir=casia_data_dir, sensitive_attr='Male', img_path_replace=True, split='all')

    lfw_casia_loader = DataLoader(lfw_casia_dataset, batch_size=200, shuffle=True)
    for i, item, in enumerate(lfw_casia_loader):
        print('i', i)
        x, u, a, s = item
